def sum_of_digits(n):
    assert int(n) == n and n >= 0, "n should be a positive integer" #edge cases where infinite loop occur
    if n < 10: #base condition to avoid infinite recursive calls, if n is less than 10 it cant be divided further down
        ...
        return sum_of_digits(division_tuple[0]) + sum_of_digits(division_tuple[1])  #recursivley call division until the digits are separated
